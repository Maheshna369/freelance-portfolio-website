...
export const HoverEffect = ({
  ...
  return (
    <div
      className={cn(
        "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 py-5 lg:py-10",
        className
      )}
    >
      {items.map((item, idx) => (
          ...
          "
              ...
              {item.github && (
                <Link href={item.github} target="_blank">
                  <button
                    className="
          relative flex-1 p-2 rounded-2xl overflow-hidden border-2 border-gray-900 dark:border-white
          dark:text-white text-gray-900 font-bold
          shadow-lg shadow-white/30 hover:bg-gray-900
          dark:hover:bg-white  hover:text-white dark:hover:text-gray-900
          transition-all duration-300
          group
        "
                  >

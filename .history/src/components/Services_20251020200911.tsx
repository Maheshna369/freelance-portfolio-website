...
const Services = () => {
  ...
  return (
    ...
    >
      ...
      {/* Cards Grid */}
      <div className="w-full flex flex-wrap justify-between items-center gap-6 px-4">
        {services.map((service, index) => (
          <div
            key={index}
            className="flex-shrink-0 w-full sm:w-[45%] lg:w-[500px]"
          >
            <CardContainer className="w-full flex justify-center py-4">
              <CardBody className="bg-white dark:bg-gray-800 w-full h-auto p-5 rounded-xl shadow-lg hover:scale-105 transition-transform duration-300">
                ...
                <CardItem className="w-full flex flex-col sm:flex-row justify-between items-center mt-4 gap-3">
                  <Link href="#contact">
                    <button className="w-full sm:flex-1 text-white py-3 rounded-lg transition-colors font-semibold">
                      Let’s Talk
                    </button>
                  </Link>
                  <Link href="#contact">
                    <Button className="w-full sm:flex-1 py-3 rounded-lg hover:bg-green-600 transition-colors font-semibold">
                      Buy Now
                    </Button>
                  </Link>
                </CardItem>

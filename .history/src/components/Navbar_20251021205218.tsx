    ...
    <header
      className={`w-full h-20 flex justify-between items-center ${
        scrolled
          ...
          : "bg-transparent"
      } text-gray-900 dark:text-white sticky top-0 z-50 transition-all duration-300`}
    >
      {/* Branding */}
      ...
      <Tooltip.Provider delayDuration={200}>
        <nav className="flex items-center gap-6 font-medium">
          <NavItem href="/" label="Home" tooltip="Go to homepage" hash={hash} />
          <NavItem
            href="/#services"
            label="Services"
...
// 💎 Improved NavItem (hash-aware highlight)
const NavItem = ({
  href,
  label,
  tooltip,
  ...
  const currentHash = href.split("#")[1] ? `#${href.split("#")[1]}` : "";
  const isActive = hash === currentHash || (href === "/" && hash === "");

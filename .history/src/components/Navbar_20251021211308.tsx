...
const Navbar = () => {
  ...
  useEffect(() => {
    ...
    // Watch URL changes (Next.js internal + browser)
    const observer = new MutationObserver(() => updateActiveHash());
    observer.observe(document.body, { childList: false, subtree: true });

    // Fallback: also listen to native events
  ...
  useEffect(() => {
    const currentHash = router.asPath.includes("#")
      ? "#" + router.asPath.split("#")[1]
      : "#home";

    setActive(currentHash);
    console.log("Active section:", currentHash);
  }, [router.asPath]);
  useEffect(() => {
    const handleScroll = () => {
